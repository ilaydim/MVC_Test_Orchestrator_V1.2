...
class ModelCoderAgent(BaseCoderAgent):
    """
    Specialized agent for generating Model classes.
    Processes ALL model files in the scaffold directory.
    """

    def __init__(self, rag_pipeline=None, llm_client=None):
        super().__init__(rag_pipeline, llm_client)
        self.category = "model"
        print(f"[Model Coder] Initialized - will process ALL model files")

    def generate_code(self) -> Dict[str, Any]:
        """
        Generate code for ALL model files.
        No limits - processes everything in the models directory.
        ...
        print(f"\n{'='*60}")
        print(f"[MODEL CODER] Processing ALL model files")
        print(f"{'='*60}\n")
        
        architecture = self._load_architecture_data()
        
        scaffold_dir = self.scaffold_root / "models"
        
        if not scaffold_dir.exists():
            print(f"[ERROR] Models directory NOT FOUND at: {scaffold_dir}")
            return {"completed": [], "failed": [], "hallucinated": []}

        result = self._process_all_files(scaffold_dir, self.category, architecture)
        
        print(f"\n{'='*60}")
        print(f"[MODEL CODER] Complete!")
        ...
        return {
            ...
            "output_directory": str(self.generated_root / "models")
        }
    ...
    def _process_all_files(self, scaffold_dir: Path, category: str, architecture: Dict[str, Any]) -> Dict[str, List]:
        ...
        if not skeleton_files:
            print(f"[WARN] No model files found in {scaffold_dir}")
            return {'completed': completed, 'failed': failed, 'hallucinated': hallucinated}
        
        print(f"[MODELS] Processing {len(skeleton_files)} files...\n")
        ...
        for idx, skeleton_file in enumerate(skeleton_files, 1):
            ...
            try:
                ...
                print(f"\n[Model Coder] 🔄 Generating: {skeleton_file.name} ({idx}/{len(skeleton_files)})...", flush=True)
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        print(f"[Model Coder] ❌ [ERROR] {error}", flush=True)
                        print(f"[Model Coder] ⚠️ Skipping {skeleton_file.name}, continuing with next file...", flush=True)
                        failed.append((skeleton_file, error))
                        new_code = None
                        break
                    ...
                    if attempt < max_retries - 1:
                        ...
                        enhanced_prompt = prompt + f"\n\n⚠️ CRITICAL: The class name MUST be exactly '{expected_class_name}'. Do NOT use any other variation. The class definition must be: class {expected_class_name}:"
                ...
                # Write to generated_src/models/
                output_dir = self.generated_root / "models"
                output_file = output_dir / skeleton_file.name
                
                output_file.write_text(new_code, encoding="utf-8")
                completed.append(output_file)
                print(f"[Model Coder] ✅ Successfully generated and verified: {skeleton_file.name}", flush=True)
                ...
                print(f"\n[Model Coder] Stopped at: {skeleton_file.name}", file=sys.stderr)
                print(f"[Model Coder] Completed so far: {len(completed)} files", file=sys.stderr)
                print(f"[Model Coder] Remaining: {len(skeleton_files) - idx} files", file=sys.stderr)
                print(f"\n💡 Solution: Wait for quota reset or use new API key", file=sys.stderr)
                print(f"{'='*60}\n", file=sys.stderr)
                break  # Exit loop, return what we have
                
            except LLMConnectionError as lce:
                # Connection error - log and skip this file
                ...
                print(f"[Model Coder] ❌ [ERROR] {error}", flush=True)
                print(f"[Model Coder] DEBUG ERROR: {type(e).__name__}: {str(e)}", flush=True)
                print(f"[Model Coder] ⚠️ Skipping {skeleton_file.name}, continuing with next file...", flush=True)
                failed.append((skeleton_file, error))
