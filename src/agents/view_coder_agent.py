...
class ViewCoderAgent(BaseCoderAgent):
    """
    Specialized agent for generating View classes.
    Processes ALL view files in the scaffold directory.
    """

    def __init__(self, rag_pipeline=None, llm_client=None):
        super().__init__(rag_pipeline, llm_client)
        self.category = "view"
        print(f"[View Coder] Initialized - will process ALL view files")

    def generate_code(self) -> Dict[str, Any]:
        """
        Generate code for ALL view files.
        No limits - processes everything in the views directory.
        ...
        print(f"\n{'='*60}")
        print(f"[VIEW CODER] Processing ALL view files")
        print(f"{'='*60}\n")
        
        architecture = self._load_architecture_data()
        
        scaffold_dir = self.scaffold_root / "views"
        
        if not scaffold_dir.exists():
            print(f"[ERROR] Views directory NOT FOUND at: {scaffold_dir}")
            return {"completed": [], "failed": [], "hallucinated": []}

        result = self._process_all_files(scaffold_dir, self.category, architecture)
        
        print(f"\n{'='*60}")
        print(f"[VIEW CODER] Complete!")
        ...
        return {
            ...
            "output_directory": str(self.generated_root / "views")
        }
    ...
    def _process_all_files(self, scaffold_dir: Path, category: str, architecture: Dict[str, Any]) -> Dict[str, List]:
        ...
        if not skeleton_files:
            print(f"[WARN] No view files found in {scaffold_dir}")
            return {'completed': completed, 'failed': failed, 'hallucinated': hallucinated}
        
        print(f"[VIEWS] Processing {len(skeleton_files)} files...\n")
        ...
        for idx, skeleton_file in enumerate(skeleton_files, 1):
            ...
            try:
                ...
                print(f"\n[View Coder] 🔄 Generating: {skeleton_file.name} ({idx}/{len(skeleton_files)})...", flush=True)
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        print(f"[View Coder] ❌ [ERROR] {error}", flush=True)
                        print(f"[View Coder] ⚠️ Skipping {skeleton_file.name}, continuing with next file...", flush=True)
                        failed.append((skeleton_file, error))
                        new_code = None
                        break
                    ...
                    if attempt < max_retries - 1:
                        ...
                        enhanced_prompt = prompt + f"\n\n⚠️ CRITICAL: The class name MUST be exactly '{expected_class_name}'. Do NOT use '{expected_class_name.replace('View', '')}' or any other variation. The class definition must be: class {expected_class_name}:"
                        prompt = enhanced_prompt
                        continue
                
                # If still invalid after retries, mark as hallucinated
                if not is_valid or not new_code:
                    print(f" ❌ FAILED: {validation_msg.split(':')[-1].strip()}")
                    hallucinated.append((skeleton_file, validation_msg))
                    continue
                
                # Write to generated_src/views/
                output_dir = self.generated_root / "views"
                output_file = output_dir / skeleton_file.name
                
                output_file.write_text(new_code, encoding="utf-8")
                completed.append(output_file)
                print(f"[View Coder] ✅ Successfully generated and verified: {skeleton_file.name}", flush=True)
                ...
                print(f"\n[View Coder] Stopped at: {skeleton_file.name}", file=sys.stderr)
                print(f"[View Coder] Completed so far: {len(completed)} files", file=sys.stderr)
                print(f"[View Coder] Remaining: {len(skeleton_files) - idx} files", file=sys.stderr)
                ...
                print(f"[View Coder] ❌ [ERROR] {error}", flush=True)
                print(f"[View Coder] DEBUG ERROR: {type(lce).__name__}: {str(lce)}", flush=True)
                print(f"[View Coder] ⚠️ Skipping {skeleton_file.name}, continuing with next file...", flush=True)
                failed.append((skeleton_file, error))
                continue  # Continue to next file
                
            except KeyboardInterrupt:
                # User interrupted - stop gracefully
                print(f"\n[View Coder] ⚠️ Interrupted by user", flush=True)
                break
                
            except Exception as e:
                # Unexpected error - log with FULL debug info and skip, continue to next file
                import traceback
                error = f"Unexpected error: {str(e)}"
                print(f"[View Coder] ❌ [ERROR] {error}", flush=True)
                print(f"[View Coder] DEBUG ERROR: {type(e).__name__}: {str(e)}", flush=True)
                print(f"[View Coder] File: {skeleton_file.name}", flush=True)
                print(f"[View Coder] Traceback:", flush=True)
                traceback.print_exc()
                print(f"[View Coder] ⚠️ Skipping {skeleton_file.name}, continuing with next file...", flush=True)
                failed.append((skeleton_file, error))
                continue  # Continue to next file
